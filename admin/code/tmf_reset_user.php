...
echo '<label class="input-group-text" for="group_id">'.$l['w_group'].'</label>'.K_NEWLINE;
echo '</div>'.K_NEWLINE;
echo '<div class="input-group-prepend">'.K_NEWLINE;
// echo '<span class="formw d-block">'.K_NEWLINE;
echo '<select class="dropdown-toggle btn btn-light" name="group_id" id="group_id" size="0" onchange="document.getElementById(\'form_userselect\').submit()">'.K_NEWLINE;
...
echo '<input type="text" name="searchterms" class="form-control" id="searchterms" value="'.htmlspecialchars($searchterms, ENT_COMPAT, $l['a_meta_charset']).'" size="20" maxlength="255" title="'.$l['w_search'].'" />';
echo '<div class="input-group-append">'.K_NEWLINE;
F_submit_button_alt('search', $l['w_search'], $l['w_search'], "btn btn-primary");
echo '</div>'.K_NEWLINE;
echo '</div></div></div>'.K_NEWLINE;
